...
if (!process.argv[2]) {
  console.error("用法：node scripts/run-scope-acceptance.mjs <epub/txt 路径> [章数，默认 2]");
  process.exit(1);
}
...
const registry = join(await mkdtemp(join(tmpdir(), "lightee-scope-reg-")), "workspaces.json");
...
const child = spawn(electronPath, [appRoot], {
  ...
  env: {
    ...
    LIGHTEE_HEADLESS_SCRIPT: join(appRoot, "scripts", "scope-acceptance.mjs"),
    LIGHTEE_SCOPE_SOURCE: sourcePath,
    LIGHTEE_SCOPE_CHAPTERS: process.argv[3] ?? "2",
  },
});
